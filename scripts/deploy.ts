...
async function main() {
  ...
  if (isTestnet || isFork) {
    ...
    if (
      !jusdAddresses.juiceDollar ||
      jusdAddresses.juiceDollar === "0x0000000000000000000000000000000000000000"
    ) {
      throw new Error(
    ...
    permit2Address = process.env.PERMIT2_ADDRESS || DEFAULT_PERMIT2_ADDRESS;
    ...
    console.log(
      `   JUSD:           ${baseAssetAddress} (from @juicedollar/jusd)`,
    );
    console.log(
      `   Permit2:        ${permit2Address} (${process.env.PERMIT2_ADDRESS ? "from PERMIT2_ADDRESS" : "default"})`,
    );
    console.log(
      `   V2 Router:      ${routerAddress} (from @juiceswapxyz/sdk-core)`,
    );
    console.log(
      `   V2 Factory:     ${v2FactoryAddress} (from @juiceswapxyz/sdk-core)`,
    );
    console.log(
      `   Init Code Hash: ${initCodeHash} (from @juiceswapxyz/sdk-core)`,
    );
  } else if (isMainnet) {
    console.log("\n🌐 Mainnet Deployment");
    ...
    console.log(
      "⚠️  To use real contracts, use --network forkTestnet or --network forkMainnet",
    );
  ...
  if (isTestnet || isMainnet || isFork) {
    const permit2Code = await ethers.provider.getCode(permit2Address);
    if (permit2Code === "0x") {
      throw new Error(
        `❌ Permit2 not deployed at ${permit2Address} on chain ${chainId}.\n` +
          `   Deploy Permit2 or set PERMIT2_ADDRESS to the chain's verified Permit2 contract.`,
      );
    }
  }
  ...
  console.log(
    `   Initial Virtual Base Reserves: ${ethers.formatUnits(initialVirtualBaseReserves, decimals)} (${decimals} decimals)`,
  );
  ...
  console.log(
    `   V2 Liquidity: ~$${totalLiquidity.toFixed(2)} (${((totalLiquidity / marketCapAtGrad) * 100).toFixed(1)}% of MC)`,
  );
  ...
  console.log(
    "   Initial Virtual Base:",
    ethers.formatUnits(initialVirtualBaseReserves, decimals),
  );
  console.log("   Init Code Hash:", initCodeHash);
  ...
  if (isTestnet) {
    ...
    console.log(
      `   factory.createToken("Test Token", "TEST", "ipfs://QmMetadataHash...")`,
    );
  ...
  if (isTestnet || isMainnet) {
    ...
    console.log(
      `npx hardhat verify --network ${networkName} ${factoryAddress} "${implementationAddress}" "${routerAddress}" "${baseAssetAddress}" "${permit2Address}" "${feeRecipient}" "${initialVirtualBaseReserves}" "${initCodeHash}"\n`,
    );
  }

...
describe("BondingCurveToken - Trading", function () {
  ...
  describe("Buy Function", function () {
    it("Should buy tokens successfully", async function () {
      ...
      await baseAsset
        .connect(user1)
        .approve(await token.getAddress(), buyAmount);
    ...
    it("Should deduct 1% fee from input", async function () {
      ...
      expect(reservesAfter.realBase).to.equal(
        reservesBefore.realBase + buyAmountAfterFee,
      );
    });
    ...
    it("Should update real reserves correctly", async function () {
      ...
      expect(reservesAfter.realToken).to.equal(
        reservesBefore.realToken - tokensOut,
      );

      // Real base reserves should increase (minus fee)
      const fee = (buyAmount * 100n) / 10000n;
      expect(reservesAfter.realBase).to.equal(
        reservesBefore.realBase + (buyAmount - fee),
      );
    });
    ...
    it("Should handle multiple sequential buys", async function () {
      const { token, baseAsset, user1, user2 } =
        await networkHelpers.loadFixture(deployFixture);
  ...
  describe("Sell Function", function () {
    ...
    it("Should sell tokens successfully", async function () {
      const { token, user1 } =
        await networkHelpers.loadFixture(buyTokensFixture);
    ...
    it("Should update real reserves correctly", async function () {
      ...
      expect(reservesAfter.realToken).to.equal(
        reservesBefore.realToken + sellAmount,
      );

      // Real base reserves should decrease
      expect(reservesAfter.realBase).to.equal(
        reservesBefore.realBase - baseOut,
      );
    });
  ...
  describe("Front-Running Protection", function () {
    ...
    it("Should compute correct pair address", async function () {
      const { token, v2Factory, baseAsset } =
        await networkHelpers.loadFixture(deployFixture);
      ...
      await v2Factory.createPair(
        await token.getAddress(),
        await baseAsset.getAddress(),
      );
      const actualPair = await v2Factory.getPair(
        await token.getAddress(),
        await baseAsset.getAddress(),
      );

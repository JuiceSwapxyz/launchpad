...
describe("TokenFactory", function () {
  ...
  async function deployFixture() {
    ...
    const baseAsset = await ethers.deployContract("MockERC20", [
      "Wrapped cBTC",
      "WcBTC",
    ]);
  ...
  describe("Deployment", function () {
    ...
    it("Should set the correct router address", async function () {
      const { factory, router } =
        await networkHelpers.loadFixture(deployFixture);
      expect(await factory.uniswapV2Router()).to.equal(
        await router.getAddress(),
      );
    });
    ...
    it("Should set the correct Permit2 address", async function () {
      const { factory, permit2 } =
        await networkHelpers.loadFixture(deployFixture);
      expect(await factory.permit2()).to.equal(await permit2.getAddress());
    });
    ...
    it("Should revert if implementation is zero address", async function () {
      const { router, baseAsset, permit2, owner, initCodeHash } =
        await networkHelpers.loadFixture(deployFixture);
      await expect(
    ...
    it("Should revert if router is zero address", async function () {
      const { implementation, baseAsset, permit2, owner, initCodeHash } =
        await networkHelpers.loadFixture(deployFixture);
      await expect(
        ethers.deployContract("TokenFactory", [
          await implementation.getAddress(),
    ...
    it("Should revert if base asset is zero address", async function () {
      const { implementation, router, permit2, owner, initCodeHash } =
        await networkHelpers.loadFixture(deployFixture);
      await expect(
        ethers.deployContract("TokenFactory", [
          await implementation.getAddress(),
    ...
    it("Should revert if Permit2 is zero address", async function () {
      const { implementation, router, baseAsset, owner, initCodeHash } =
        await networkHelpers.loadFixture(deployFixture);
      await expect(
        ethers.deployContract("TokenFactory", [
          await implementation.getAddress(),
    ...
    it("Should revert if fee recipient is zero address", async function () {
      const { implementation, router, baseAsset, permit2, initCodeHash } =
        await networkHelpers.loadFixture(deployFixture);
      await expect(
        ethers.deployContract("TokenFactory", [
          await implementation.getAddress(),
  ...
  describe("Token Creation", function () {
    ...
    it("Should create multiple tokens", async function () {
      ...
      const token2 = await ethers.getContractAt(
        "BondingCurveToken",
        await factory.getToken(1),
      );
      const token3 = await ethers.getContractAt(
        "BondingCurveToken",
        await factory.getToken(2),
      );
    ...
    it("Should track correct creator for each token", async function () {
      const { factory, user1, user2, baseAsset } =
        await networkHelpers.loadFixture(deployFixture);

      await factory
        .connect(user1)
        .createToken("User1 Token", "U1", "ipfs://QmUser1");
      await factory
        .connect(user2)
        .createToken("User2 Token", "U2", "ipfs://QmUser2");
    ...
    it("Should accept various metadata URI formats", async function () {
      ...
      // IPFS
      await expect(
        factory.createToken("Token1", "TK1", "ipfs://QmTest123"),
      ).to.not.be.revertedWith("");

      // Arweave
      await expect(
        factory.createToken("Token2", "TK2", "ar://abc123def"),
      ).to.not.be.revertedWith("");
    ...
    it("Should handle long metadata URIs", async function () {
      ...
      await expect(
        factory.createToken("Test Token", "TEST", longMetadata),
      ).to.not.be.revertedWith("");
    });
  ...
  describe("Dev Buy Creation", function () {
    async function buildPermit(
      fixture: Awaited<ReturnType<typeof deployFixture>>,
      amount: bigint,
      overrides: any = {},
    ) {
      const spender = overrides.spender ?? (await fixture.factory.getAddress());
      const token = overrides.token ?? (await fixture.baseAsset.getAddress());
      const expiration = overrides.expiration ?? 4_000_000_000;
      const sigDeadline = overrides.sigDeadline ?? 4_000_000_000;

      return {
        details: {
          token,
          amount: overrides.permitAmount ?? amount,
          expiration,
    ...
    it("Should create a token and execute the creator dev buy atomically with Permit2", async function () {
      const fixture = await networkHelpers.loadFixture(deployFixture);
      const { factory, baseAsset, permit2, user1 } = fixture;

      const devBuyBaseIn = ethers.parseEther("10");
      const permit = await buildPermit(fixture, devBuyBaseIn);

      await baseAsset.mint(user1.address, devBuyBaseIn);
      await baseAsset
        .connect(user1)
        .approve(await permit2.getAddress(), devBuyBaseIn);

      const tx = await factory
        .connect(user1)
        .createTokenWithDevBuyPermit(
          "Dev Token",
          "DEV",
          "ipfs://QmDev",
          devBuyBaseIn,
      ...
      const tokensOut = await token.balanceOf(user1.address);
      const reserves = await token.getReserves();
      const fee = (devBuyBaseIn * 100n) / 10000n;

      expect(tokensOut).to.be.gt(0);
      expect(await baseAsset.balanceOf(tokenAddress)).to.equal(devBuyBaseIn);
      expect(reserves.realBase).to.equal(devBuyBaseIn - fee);
      expect(await token.launchFinalized()).to.equal(true);
      expect(await token.devBuyExecuted()).to.equal(true);

      await expect(tx)
        .to.emit(factory, "DevBuyExecuted")
        .withArgs(tokenAddress, user1.address, devBuyBaseIn, tokensOut);
      await expect(tx)
        .to.emit(token, "Buy")
        .withArgs(
          user1.address,
          devBuyBaseIn,
          tokensOut,
          await token.virtualTokenReserves(),
          await token.virtualBaseReserves(),
        );
    });

    it("Should keep TokenCreated before the dev-buy events in the same transaction", async function () {
      const fixture = await networkHelpers.loadFixture(deployFixture);
      const { factory, baseAsset, permit2, user1 } = fixture;
      const devBuyBaseIn = ethers.parseEther("5");
      const permit = await buildPermit(fixture, devBuyBaseIn);
      ...
      const tokenCreatedLog =
        receipt?.logs.findIndex((log: any) => {
          try {
      ...
      const buyLog =
        receipt?.logs.findIndex((log: any) => {
          try {
            return token.interface.parseLog(log)?.name === "Buy";
          } catch {
            return false;
          }
        }) ?? -1;
      const devBuyLog =
        receipt?.logs.findIndex((log: any) => {
          try {
            return factory.interface.parseLog(log)?.name === "DevBuyExecuted";
          } catch {
            return false;
          }
        }) ?? -1;

      expect(tokenCreatedLog).to.be.gte(0);
      expect(buyLog).to.be.gt(tokenCreatedLog);
      expect(devBuyLog).to.be.gt(buyLog);
    });

    it("Should revert when the dev buy exceeds the 20% curve-supply cap", async function () {
      const fixture = await networkHelpers.loadFixture(deployFixture);
      const { factory, baseAsset, permit2, user1 } = fixture;
      const devBuyBaseIn = ethers.parseEther("1000");
      const permit = await buildPermit(fixture, devBuyBaseIn);
    ...
    it("Should revert the full creation when slippage protection fails", async function () {
      const fixture = await networkHelpers.loadFixture(deployFixture);
      const { factory, baseAsset, permit2, user1 } = fixture;
      const devBuyBaseIn = ethers.parseEther("10");
      const permit = await buildPermit(fixture, devBuyBaseIn);
    ...
    it("Should reject permits for the wrong token or spender", async function () {
      const fixture = await networkHelpers.loadFixture(deployFixture);
      ...
      await expect(
        factory
          ...
          .createTokenWithDevBuyPermit(
            ...
            devBuyBaseIn,
            0,
            await buildPermit(fixture, devBuyBaseIn, { token: user1.address }),
            "0x",
          ),
      ).to.be.revertedWithCustomError(factory, "PermitTokenMismatch");

      await expect(
        factory.connect(user1).createTokenWithDevBuyPermit(
          "Wrong Spender",
          "WSP",
          "ipfs://QmWrongSpender",
          devBuyBaseIn,
          0,
          await buildPermit(fixture, devBuyBaseIn, {
            spender: user1.address,
          }),
          "0x",
        ),
      ).to.be.revertedWithCustomError(factory, "PermitSpenderMismatch");
    });

    it("Should reject undersized and expired permits", async function () {
      const fixture = await networkHelpers.loadFixture(deployFixture);
      const { factory, user1 } = fixture;
      const devBuyBaseIn = ethers.parseEther("10");
      ...
      await expect(
        factory.connect(user1).createTokenWithDevBuyPermit(
          ...
          0,
          await buildPermit(fixture, devBuyBaseIn, {
            permitAmount: devBuyBaseIn - 1n,
          }),
          "0x",
        ),
      ).to.be.revertedWithCustomError(factory, "PermitAmountTooLow");
      ...
      ).to.be.revertedWithCustomError(factory, "PermitExpired");
    });

    it("Should prevent non-factory callers from executing a dev buy directly", async function () {
      const { factory, user1 } =
        await networkHelpers.loadFixture(deployFixture);

      await factory.createToken("Plain Token", "PLN", "ipfs://QmPlain");
      const token = await ethers.getContractAt(
        "BondingCurveToken",
        await factory.getToken(0),
      );
      ...
      await expect(
        token
          .connect(user1)
          .factoryDevBuy(user1.address, ethers.parseEther("1"), 0),
  ...
  describe("Initial Virtual Base Reserves Management", function () {
    it("Should set the correct initial virtual base reserves in constructor", async function () {
      const { factory } = await networkHelpers.loadFixture(deployFixture);
      expect(await factory.initialVirtualBaseReserves()).to.equal(
        ethers.parseEther("4500"),
      );
    });
    ...
    it("Should allow owner to update initial virtual base reserves", async function () {
      ...
      await expect(
        factory.connect(owner).setInitialVirtualBaseReserves(newValue),
      )
    ...
    it("Should revert if non-owner tries to update initial virtual base reserves", async function () {
      ...
      await expect(
        factory
          .connect(user1)
          .setInitialVirtualBaseReserves(ethers.parseEther("5000")),
      ).to.be.revertedWithCustomError(factory, "OwnableUnauthorizedAccount");
    });
    ...
    it("Should not affect existing tokens when updated", async function () {
      ...
      await factory
        .connect(owner)
        .setInitialVirtualBaseReserves(ethers.parseEther("6000"));
